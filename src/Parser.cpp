...
Parser::ParseList() {
    ...
    while (true) {
        ...
        default:
            ...
            } while (m_lexer.Peek().type != Token::Type::RightParen and
                     m_lexer.Peek().type != Token::Type::Eof);
...
Value
Parser::ParseInteger(const Token &token) {
...
Value
Parser::ParseFloat(const Token &token) {
