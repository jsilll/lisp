...
bool
Lexer::IsPunctuation(const char c) noexcept {
    return c == '(' or c == ')' or c == '\'' or c == '"' or c == ';';
}

void
Lexer::AdvanceChar() noexcept {
    ++m_index;
    ++m_position.column;
}

void
Lexer::AdvanceNewline() noexcept {
    ++m_index;
    m_position.AdvanceNewline();
}

void
Lexer::SkipWhitespace() noexcept {
...
Token
Lexer::ScanSingleChar(const Token::Type type,
                      const std::string_view &lex) noexcept {
...
Token
Lexer::ScanSymbol() noexcept {
    const auto start = m_index;
    const auto pos = m_position;
    while (m_index < m_source.size() and !IsPunctuation(m_source[m_index]) and
           !std::isspace(m_source[m_index])) {
        AdvanceChar();
    }
    const auto lex = std::string_view(&m_source[start], m_index - start);
    return lex.size() == 0 ? Token{Token::Type::Invalid, pos, lex}
                           : Token{Token::Type::Symbol, pos, lex};
}

Token
Lexer::ScanString() noexcept {
    AdvanceChar();
    const auto start = m_index;
    const auto pos = m_position;
    while (m_index < m_source.size() and m_source[m_index] != '"') {
